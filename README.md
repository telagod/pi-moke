...
<p align="center">
  <strong>一套调校过的 <a href="https://github.com/earendil-works/pi-mono">pi</a> 形态，一键交给任何人。</strong><br/>
  终端 coding agent 的可复现发行版：人格、技能、快压、Goal、记忆审查。
</p>
...
安装脚本：补依赖 → 装 pi → 启动优化 → 写入人格 → 合并 settings → 落盘 Goal / hermes 配置 → 钉技能包。

**不写入 provider、API key、auth.json。**
...
| 窗况 | 每条新结果带 `[ctx %]`。`context({op:"status"})` 列出 raw 大块；换题、大读前自判。 |
...
| 启动 | 有 bun 用 bun，无则 node；`--max-old-space-size=512`；验证失败回退 |
| 密钥 | 零密钥发行。模型与 provider 客自配。 |
| 安装 | Linux / macOS / WSL2 / Termux 一条路。幂等；旧 `AGENTS.md` 自动备份。 |

话少，手快，事毕交差。盘缠是 token。
...
| 平台 | 状态 | 说明 |
| --- | --- | --- |
| Linux | 支持 | 只需 `curl`；node 缺失或 &lt; 22 时自动装 |
| macOS | 支持 | 无需 brew；`sed` / `readlink` 已兼容 |
| WSL2 | 支持 | Windows 推荐路径 |
| Termux | 支持 | `pkg install curl git` 后直跑 |
| Windows 原生 | 有限 | 官方要 bash；建议 WSL2 |
...
Goal 轮次帽：`~/.pi/agent/pi-goal.json` → `continuationLimits.automaticTurns`（`null` = 不设限）。
